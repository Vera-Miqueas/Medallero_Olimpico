...
void impDeportesInd(int indice) {
	cout << deportes[indicesIndividuales[indice]-1] << endl;
}

void impDeportesCol(int indice) {
	cout << deportes[indicesColectivos[indice]-1] << endl;
}

int	convertidorIdDeporte(int IdListado, char tipo) {
	int retorno;
	if (tipo == 'i') {
		retorno = indicesIndividuales[IdListado - 1];
	}
	else if (tipo == 'c') {
		retorno = indicesColectivos[IdListado - 1];		
	}
	return retorno;
}

void impDeporte(int indice) {
	indice++;
	if (indice >= 1 && indice <= totalDeportes) {
		cout << deportes[indice - 1] << endl;
	} else {
